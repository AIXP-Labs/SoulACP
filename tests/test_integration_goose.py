...
Run with: pytest tests/test_integration_goose.py -v
...
def client_class():
    ...
    return resolve_client_class("goose")


def test_connect_and_query(config, client_class):
    """Test basic Goose connection and query."""
...
def test_multi_turn(config, client_class):
    """Test Goose multi-turn conversation."""
    from soulacp import ACPConnectionPool
    ...
    async def run():
        ...
        try:
            async with pool.acquire() as (client, session_id):
                r1 = await client.query("Remember the number 23.")
                assert len(r1) > 0
