"""Integration tests for Copilot Language Server adapter.

Run with: pytest tests/test_integration_copilot_ls.py -v
...
pytestmark = pytest.mark.skipif(
    find_copilot_ls_binary() is None,
    reason="Copilot Language Server not installed",
)
...
def client_class():
    ...
    return resolve_client_class("copilot-ls")


def test_connect_and_query(config, client_class):
    """Test basic Copilot Language Server connection and query."""
...
def test_multi_turn(config, client_class):
    """Test Copilot Language Server multi-turn conversation."""
    from soulacp import ACPConnectionPool
    ...
    async def run():
        ...
        try:
            async with pool.acquire() as (client, session_id):
                r1 = await client.query("Remember the number 21.")
                assert len(r1) > 0
