"""Integration tests for Cline ACP adapter.

Run with: pytest tests/test_integration_cline.py -v
Skip if cline not available.
"""
...
pytestmark = pytest.mark.skipif(
    find_cline_binary() is None,
    reason="Cline CLI not installed",
)
...
def client_class():
    ...
    return resolve_client_class("cline")


def test_connect_and_query(config, client_class):
    """Test basic Cline connection and query."""
...
def test_multi_turn(config, client_class):
    """Test Cline multi-turn conversation."""
    from soulacp import ACPConnectionPool
    ...
    async def run():
        ...
        try:
            async with pool.acquire() as (client, session_id):
                r1 = await client.query("Remember the number 55.")
                assert len(r1) > 0
